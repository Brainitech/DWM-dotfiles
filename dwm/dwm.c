...
enum { NetSupported, NetWMName, NetWMState, NetWMCheck,
       ...
       NetWMWindowTypeDialog, NetClientList, NetDesktopNames, NetDesktopViewport, NetNumberOfDesktops, NetCurrentDesktop, NetWMDesktop, NetCloseWindow, NetLast }; /* EWMH atoms */
...
drawbar(Monitor *m)
{
	...
	int x = 0, w, tw = 0, stw = 0, moveright = 0;		
			...
			case 's':
				if (m == selmon) { /* status is only drawn on selected monitor */
					...
					if (moveright) {
						...
						x = drw_text(drw, m->ww - tw - stw, 0, tw, bh, lrpad / 2 - 2, stext, 0);
...
{
	int tag_index = 0;
	while (!(c->tags & (1 << tag_index)) && (tag_index < LENGTH(tags)))
		tag_index++;
	long data[] = { tag_index };
...
centeredmaster(Monitor *m)
{
	...
	if (i < m->nmaster) {
      ...
      mfacts -= c->cfact;
		...
		if ((i - m->nmaster) % 2 ) {
			h = (m->wh - ety)  * (c->cfact / lfacts) - 2*m->gappx;
			if(m->nmaster == 0)
				resize(c, m->wx + m->gappx, m->wy + ety + m->gappx, tw - (2*c->bw) - m->gappx, h - (2*c->bw), 0);
			else
				resize(c, m->wx + m->gappx, m->wy + ety + m->gappx, tw - (2*c->bw) - m->gappx, h - (2*c->bw), 0);
			...
			h = (m->wh - oty)  * (c->cfact / rfacts) - 2*m->gappx;
...
centeredfloatingmaster(Monitor *m)
{
	...
	if (i < m->nmaster) {
		...
		resize(c, m->wx + mx + m->gappx, m->wy + my + m->gappx, w - (2*c->bw),
		       mh - (2*c->bw), 0);
		if(mx + WIDTH(c) < m->mw)	   
			mx += WIDTH(c) + m->gappx;
		mfacts -= c->cfact;
